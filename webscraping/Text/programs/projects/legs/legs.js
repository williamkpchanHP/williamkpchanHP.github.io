...
var imgNo = 1;
const imgHeader = "<img src='http://www.between-legs.com/content/galleries/";
const imgMidterm = "/full/";
const imgTailer = ".jpg'>";
...
function nextImg() {
	...
	theImgAddr = imgHeader + ImgList[listLen] + imgMidterm + FormatNumberLength(imgNo) + imgTailer;
...
function init_mypicList() {
	if (localStorage.getItem("savedLegList") === null) {
	  mypicList = [];
	  localStorage.savedLegList = JSON.stringify(mypicList);
	} else{
	  mypicList = JSON.parse(localStorage.savedLegList);
	}
}
