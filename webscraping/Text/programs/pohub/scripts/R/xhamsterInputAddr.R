...
urlHeader = 'https://xhamster.com/search?q='
keywords = readline(prompt="enter keywords: ")
newKeywords = gsub(' ' , '+', keywords)
urlTail = '&reset=1&from=video_related&p='
pageHead = paste0(urlHeader, newKeywords, urlTail)
...
filename = paste0("xhamster ",newKeywords,".html")
theWholePage = character(0)

	for(i in startNum:totalPages){
		cat(i,". ")
