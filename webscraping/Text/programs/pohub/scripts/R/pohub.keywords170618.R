...
#	thekeyword <<- iconv(thekeyword, to = "ASCII", sub = "cx")
	...
	cat("\n")
	page = 1
	groupPageNum = 800	# 4 lines each group
	while(length(theWholePage)%/%groupPageNum > 0){
		cat("writing ", page," ")
		thepage = theWholePage[1:groupPageNum]
		theWholePage= theWholePage[-(1:groupPageNum)]
		cat(length(theWholePage), "\n")

		sink(paste0("pohub ",thekeyword, " P ", page, ".html"))
	...
	sink(paste0(thekeyword, " P ", page, ".html"))
	cat("<base target=_blank>\n")
	cat("<style>body { font-size: 24px; background-color: #000000; color: #109030;}a { text-decoration: none;	color: #28B8B8;}a:visited { color: #389898;}A:hover {   color: yellow;}A:focus {   color: red;}</style>\n")
...
seekforkey <- function(thepage, seekkey){
	...
	linenum = sort(c(linenum, linenum - 7))
