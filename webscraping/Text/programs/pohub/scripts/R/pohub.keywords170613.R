...
boilPages <- function(searchKey){
	...
	for(i in 1:thenum){
		...
		theoutpage = tryCatch( 
			...
			warning = function(i){thenum = i}, 
			error = function(i) {thenum = i},
			finally = function(thepage) {
				...
				return(gsub('jpg.*' , 'jpg"></a>', thepage))
				}
			)
		theWholePage = c(theWholePage, boilit(theoutpage))
