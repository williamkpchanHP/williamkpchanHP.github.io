...
#support functions
 ...
 # askForCode
      ...
      # to confirm include desktop folder
        Selection <- readline(prompt="include desktop folder? 1/0  ")
          if(tolower(Selection) == "1") {
            ...
            desktopFolder <<- paste0("C:/Users/william/Desktop/Imp Mon Data/",Selection)
  ...
  # reload DesktopList
    ...
    saveReport <- function(){
      setwd("C:/Users/william/Desktop")  # goto desktop to avoid double quote path when using shell
      htmlFlag <<- TRUE
...
# program entry
  ...
  # load codenameTable
   ...
   # init histList
    ...
    histList = list()
    signalList = c("complex","upXAmt")
    reload()  # load histList

  # setwd("C:/Users/User/Desktop")  # to release folder
  # init variables
    maxMinRecords = matrix(, nrow = 0, ncol = 5)
    maxAmtRecords = maxMinRecords
    maxPlusMinRecords = maxMinRecords
  # loop for service
    cat(gold("\nview alarm history"))
    looping = TRUE
    while(looping){
      ...
      if(desktopFolderFlag) reloadDesktopList()  # only update data is requored
      amtSummary = character()
      pcDAmtSummary = character()

      # collect klines and pre process data strings
      klines = FetchHKKline(keyword)  # collect klines
      klines[,"Date"] = gsub("-|^..","",klines[,"Date"]) # modify date
      klines = klines[,-6] # delete "Vol"
      klines[,"Open"] = paste(" O:",klines[,"Open"]) # add legend
      klines[,"Close"] = paste(" C:",klines[,"Close"]) # add legend
      klines[,"High"] = paste(" H:",klines[,"High"]) # add legend
      klines[,"Low"] = paste(" L:",klines[,"Low"]) # add legend
      Amtstr = paste(round(as.numeric(klines[,"Amt"]),0), "w")
      klines[,"Amt"] = paste(" Amt:",Amtstr) # add legend
      ...
      for(i in 1:length(histList)){
        ...
        if(length(resultLines)==0){
          ...
          printReport(reportPage[resultLines])
          lastAmtSummary = summariseReport(reportPage[resultLines])
          amtSummary = as.numeric(amtSummary)
          pcDAmtSummary = as.numeric(pcDAmtSummary)
        }
      }
      saveReport()
      shell("viewAlarmReport.html")
    }
cat("don't forget update amtHistory.txt")
